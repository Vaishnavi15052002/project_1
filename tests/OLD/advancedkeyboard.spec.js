...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Full Name' }).fill('VAISHNAVI');
  //  Press Tab to move focus to the next input field (Email)
  await page.getByRole('textbox', { name: 'Full Name' }).press('Tab');
    //  Fill the Email field
  await page.getByRole('textbox', { name: 'name@example.com' }).fill('vaishnavi@GMAIL.COM');
  ...
  await page.getByRole('textbox', { name: 'Full Name' }).fill('KANDGULE');
   //  (Verification Point):
  await expect(page.getByRole('textbox', { name: 'Full Name' })).toHaveValue('KANDGULE');
});